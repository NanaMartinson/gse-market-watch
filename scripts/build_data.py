...
# Stock name mapping (add more as needed)
STOCK_NAMES = {
...
def build_stock_data(df, symbol):
    ...
    # Year high/low
    year_high = float(latest['year_high']) if pd.notna(latest['year_high']) else stock_df['close'].max()
    year_low = float(latest['year_low']) if pd.notna(latest['year_low']) else stock_df['close'].min()
    ...
    history = stock_df.tail(252)[['date', 'close', 'volume', 'change']].copy()
...
def main():
    ...
    for symbol in sorted(df['symbol'].unique()):
        ...
        if stock_data:
            ...
            print(f"  {symbol}: GH₵{stock_data['price']:.2f}")
